...
**Evidence classes:** `code` = committed file this session; `CI` = GitHub Actions job in `.github/workflows/`; `local GET / UX (cts-ai, 2026-09-02)` = Café Fausse App probe (J1–J8: Vite + Flask + `cafe-pg` while the DB was up; J9 / NFR-7 Vite slice: **Vite-only** `:5173`); `owner/sponsor mobile UX this session (2026-09-05)` = owner report of Safari on iPhone 16 plus A36 mobile fit on the public hosts; `App public-host Chrome/Firefox this session (2026-09-05)` = Café Fausse App handoff: Chrome **152.0.7977.83** + Firefox **155.0.1** on cts-ai desktop; five SRS routes **PASS** on `https://cafe.artof.link/`; `Unknown` = no probe this session, or a measured number that is **not** an SRS-budget claim.

[Issue #40](https://github.com/artofdream/aea-interactive-design/issues/40) recorded J1–J8 (PR #43). [Issue #44](https://github.com/artofdream/aea-interactive-design/issues/44) records J9 + the Vite-only NFR-7 slice. [Issue #117](https://github.com/artofdream/aea-interactive-design/issues/117) records the owner/sponsor mobile UX probe this session (2026-09-05). [Issue #120](https://github.com/artofdream/aea-interactive-design/issues/120) records the App Chrome + Firefox public-host **PASS** this session (2026-09-05). This Knowledge VM did not reach cts-ai Vite `:5173` / Flask `:5000`, did not run the phones, and did not open the cts-ai report files. The Journey table below is App’s 2026-09-02 handoff plus the owner report and App public-host browser handoff cited on NFR-7 / NFR-8.
...
| ID | Summary | In-repo | Evidence | Why it matters for the course |
|---|---|---|---|---|
| NFR-1 | Page load within 3 seconds on standard broadband | Intended SPA + static assets under `frontend/` and `theme.css`. | **Unknown** as an SRS-budget claim. **NFR-1 remains Unknown** — Wi‑Fi device stopwatch is not proven SRS “standard broadband”. Evidence note only — **device stopwatch (ROG / Android / Chrome / Wi‑Fi)** (2026-09-05 23:28 CEST): wall loadEvent Home **846 ms** · Menu **370 ms** · Reservations **1210 ms** (ASUS_I001DC / ROG Phone 2 / Android 10 / Chrome 152.0.7977.75 / Wi‑Fi SSID `tsarafidy` −53 dBm / CDP via adb forward). Prior local Vite home **56 ms** (cts-ai). Do not say NFR-1 is **met**. | The SRS states a 3s budget on standard broadband. A Wi‑Fi phone stopwatch is an evidence note only, not a proven standard-broadband class. |
| NFR-2 | Form submissions processed within 2 seconds | `DB_STATEMENT_TIMEOUT_MS=2000` / `DB_CONNECT_TIMEOUT` in `db.py` (fail-closed timeout, **not** a UX stopwatch). | **Unknown** as an SRS-budget claim. **NFR-2 remains Unknown** — Wi‑Fi device stopwatch is not proven SRS “standard broadband”. Evidence note only — **device stopwatch (ROG / Android / Chrome / Wi‑Fi)** (2026-09-05 23:28 CEST): reservation **242 ms** → confirmed; newsletter **156 ms** → subscribed (same device / Wi‑Fi / Chrome / CDP via adb forward as the NFR-1 note). Prior local Vite `GET /api/site` **32 ms** is not a submit stopwatch. Do not say NFR-2 is **met**. | A 2s **timeout** plus a Wi‑Fi phone submit is not evidence that the SRS “standard broadband” class is met. |
...
Café Fausse App handoff via ADB on cts-ai. **Not** an automatic **NFR-1** / **NFR-2** **met** claim. **NFR-1** / **NFR-2 remain Unknown** — Wi‑Fi device stopwatch is not proven SRS “standard broadband”.
...
| Journey / NFR recording | this page; [#40](https://github.com/artofdream/aea-interactive-design/issues/40) / [#44](https://github.com/artofdream/aea-interactive-design/issues/44) / [#117](https://github.com/artofdream/aea-interactive-design/issues/117) / [#119](https://github.com/artofdream/aea-interactive-design/issues/119) / [#120](https://github.com/artofdream/aea-interactive-design/issues/120) | App local UX / GET 2026-09-02 (cts-ai); J9 / NFR-3 / NFR-8 Vite slice **Vite-only**; `probe-nfr/` PNGs on cts-ai (not in this repo); owner/sponsor mobile UX this session (2026-09-05) on public hosts; App Chrome **152.0.7977.83** + Firefox **155.0.1** five-route **PASS** on `https://cafe.artof.link/` (dump-dom / screenshots; reports on cts-ai, not in repo); **device stopwatch (ROG / Android / Chrome / Wi‑Fi)** App handoff 2026-09-05 23:28 CEST; Knowledge GET of the 2026-09-02 tunnel timed out | J1–J8 PASS while DB was up; J9 / NFR-3 / NFR-8 PASS (Vite-only Edge screenshots + theme.css) plus public-host mobile note (iPhone 16 Safari + A36 UX fit). NFR-7 **partial** (Chrome + Firefox **PASS** on `cafe.artof.link`; Chrome-on-Android **PASS 5/5** on ROG; Safari **PASS** on iPhone 16; Edge all routes on Vite; desktop Safari not probed; A36 browser engine Unknown). **NFR-1** / **NFR-2 remain Unknown** — Wi‑Fi device stopwatch is not proven SRS “standard broadband”. Tunnel is not the live share; `cafe.artof.link` is. |
