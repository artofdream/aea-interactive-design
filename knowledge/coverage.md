...
**Café Fausse App Chrome + Firefox on the public host this session (2026-09-05)** ([#120](https://github.com/artofdream/aea-interactive-design/issues/120)): Chrome **152.0.7977.83** + Firefox **155.0.1** on cts-ai desktop. All five SRS routes **PASS** on `https://cafe.artof.link/` (`/`, `/menu`, `/gallery`, `/reservations`, `/about`). Method: Chrome headless dump-dom; Firefox headless screenshots. Reports `C:\Users\Public\cafe-nfr7-probe.json` / `.txt` on cts-ai (not in this repo). This Knowledge VM did not open those files. The 2026-09-02 Vite-only Firefox-home note stays prior evidence. **NFR-7** stays **Partial** — not a four-browser claim (desktop Safari not probed; mobile Safari only from [#117](https://github.com/artofdream/aea-interactive-design/issues/117)). **NFR-1** **met** / **PASS** is the A36 Brave owner-claimed-broadband cold Home ([#123](https://github.com/artofdream/aea-interactive-design/issues/123)), not this desktop dump-dom. NFR-2 stays **Unknown**. ROG Wi‑Fi device stopwatch ([#119](https://github.com/artofdream/aea-interactive-design/issues/119) / [#122](https://github.com/artofdream/aea-interactive-design/pull/122)) stays a separate evidence note — not the broadband **met** claim.
...
| NFR-2 | Form submissions processed within 2 seconds | `DB_STATEMENT_TIMEOUT_MS=2000` / `DB_CONNECT_TIMEOUT` in `db.py` (fail-closed timeout, **not** a UX stopwatch). | NFR-2 stays **Unknown**. Form submit was **not observed** on the A36 Brave broadband take (recording ended mid `/reservations`; no success UI). Do not invent submit timing. ROG Wi‑Fi reservation **242 ms** → confirmed and newsletter **156 ms** → subscribed ([#119](https://github.com/artofdream/aea-interactive-design/issues/119) / [#122](https://github.com/artofdream/aea-interactive-design/pull/122)) remain a Wi‑Fi evidence note — not a broadband **met**. Prior local Vite `GET /api/site` **32 ms** is not a submit stopwatch. | A 2s **timeout** plus a Wi‑Fi phone submit is not evidence that the SRS form-submit budget is met on standard broadband. |
...
These numbers stay a local evidence note. They are **not** the **NFR-1** **met** / **PASS**. That claim is the A36 Brave owner-claimed-broadband cold Home below. NFR-2 stays **Unknown**.

### Device stopwatch (ROG / Android / Chrome / Wi‑Fi) — evidence note only

Café Fausse App handoff via ADB on cts-ai. **Not** the broadband **met** claim. This ROG Wi‑Fi stopwatch stays a **separate** evidence note ([#119](https://github.com/artofdream/aea-interactive-design/issues/119) / [#122](https://github.com/artofdream/aea-interactive-design/pull/122)). **NFR-1** **met** / **PASS** is the A36 Brave owner-claimed-broadband cold Home ([#123](https://github.com/artofdream/aea-interactive-design/issues/123)). NFR-2 stays **Unknown** (ROG submits are Wi‑Fi, not broadband).
...
Café Fausse App handoff. Owner-claimed broadband + measured cold Home on `https://cafe.artof.link/`. This is the honest **NFR-1** **met** / **PASS** for the SRS ≤3s page-load budget. NFR-2 stays **Unknown** — form submit was not observed this take. Do not invent submit timing. Do not invent contents of the cts-ai report file.
...
| URL | `cafe.artof.link` |
| Form submit | not observed this take — NFR-2 stays **Unknown** |
