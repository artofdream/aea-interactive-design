...
**In plain English:** This is the pack for the recording / talk-cut session. Pick one ~10 minute cut. Clips, shot list, and the full talk tracks stay on their source pages. This page does not replace them.
...
**Grade floor:** official SRS only — **FR-1..FR-18**, **NFR-1..NFR-9**. Do not invent **FR-19**. Do not say **NFR-1** / **NFR-2** are met. **NFR-7** is **partial**. `/operator` is a read-only recording helper — **not FR-19**.

## Open these

- [Talk cuts](presentation.md) — three variants (layers / architecture / coding) plus the shared open and close. Spoken timing lives there.
...
## What to film (summary)

Prefer live share `https://cafe.artof.link/` (Lightsail staging [#57](https://github.com/artofdream/aea-interactive-design/issues/57) — weekend window, not forever production). Knowledge map: `https://knowledge.cafe.artof.link/`. Clips on the [Video script](video-script.md) and [Brief](brief.md) are the fallback look if staging drops.

Exact click paths stay on [Must-film shots](must-film-shots.md). Do not move those shots here.
