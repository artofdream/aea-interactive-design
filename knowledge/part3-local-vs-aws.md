...
**This table is camera-safe:** **Local (dev)** · Vite + Flask + local Postgres vs **MSAIE staging** at **cafe.artof.link** (tip `73d202d`). Host-vendor / machine nicknames are not in the table — see the footnote below. No FR/NFR IDs required here.
...
| --- | --- | --- |
| **What was used** | **Local (dev)** · Vite + Flask + local Postgres | Deployed staging at **cafe.artof.link** — MSAIE project staging |
...
| **Host tip (current)** | Developer machine | **cafe.artof.link** tip **`73d202d`** (presentation host) |
...
*Off-camera footnote (not the on-camera table): host vendor Lightsail, developer-box nickname cts-ai, and Windows clone path `C:\projects\code\aea-interactive-design` live here and on [Honesty](honesty.md). Do not put those names in the table or on camera.*

---

## Architecture point (one line)

**Same design, two deploy targets** — Local (dev) for iterate; cafe.artof.link (MSAIE staging) to prove the same React→Flask→Postgres stack on a public HTTPS host — so the HLD holds across environments.

---

## Architect questions (why/how — on-camera answers live in NATURAL VO)

Talk spine: Part 2 UX/business · **Part 3 architecture why/how** · Part 4 coding why/how · Part 5 honesty/close. Camera-safe deploy table above; host-vendor nicknames stay in the footnote.
...
| **Why two deploy targets?** | Same design: Local (dev) · Vite + Flask + local Postgres for iterate; MSAIE staging at cafe.artof.link for shared HTTPS demo. Temporary staging — not production forever. |
...
**On camera avoid:** “weekend Lightsail staging”, heavy host-vendor / Route53 / Caddy ops jargon.  
**Main table is camera-safe.** Off-camera footnote / [Honesty](honesty.md) may name host vendor, machine nickname, and clone path.
...
*Packed 2026-09-07 Europe/Berlin · PROTOTYPE · Honest cut: Local (dev) · Vite + Flask + local Postgres · MSAIE staging at cafe.artof.link tip 73d202d · Caddy TLS → Flask → on-box Postgres · NATURAL track architect lens (MSAIE / cafe.artof.link on camera; local vs AWS as deploy decision).*
