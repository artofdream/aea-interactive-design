...
| Knowledge host HTTPS **GET 200** (2026-09-02 Europe/Berlin): `knowledge.cafe.artof.link` | **NFR-7** four-browser claim — Chrome / Safari **Unknown**. Partial only: Edge all routes + Firefox home (Vite-only) |
| HTTP `knowledge.cafe.artof.link` → **301** to HTTPS; Pages `https_enforced=true` | `cafe.artof.link` as Café Fausse App — **not our restaurant** (CNAME to an AWS ELB) |
| Journey **J1–J8 PASS** (cts-ai, DB up) + **J9 PASS** (Vite-only viewports + theme) — [Coverage](coverage.md) / [#40](https://github.com/artofdream/aea-interactive-design/issues/40) / [#44](https://github.com/artofdream/aea-interactive-design/issues/44) | Mentioned tunnel `https://nine-teams-try.loca.lt/` — Knowledge GET **timed out** this session; do not claim it is live |
...
6. **Honesty line.** J1–J8 PASS is local cts-ai UX while the DB was up. J9 **PASS** is Vite-only (viewports + theme) — not Flask+Postgres. **NFR-7** is **partial** (Edge + Firefox home; Chrome/Safari Unknown). **NFR-1** / **NFR-2** stay not-claimed-met. Do not say the SRS broadband budgets are met.
...
3. Say J1–J8 **PASS** (cts-ai, DB up) and J9 **PASS** (Vite-only viewports + theme) from [Coverage](coverage.md) / [#40](https://github.com/artofdream/aea-interactive-design/issues/40) / [#44](https://github.com/artofdream/aea-interactive-design/issues/44). Say **NFR-7** is **partial** (Edge + Firefox home; Chrome/Safari Unknown). Say not-claimed-met for **NFR-1** / **NFR-2**.
