...
**App tunnel this session:** `https://happy-glasses-film.loca.lt/` — GET **200** (SPA HTML). `https://happy-glasses-film.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. **Not** `cafe.artof.link`. Clips stay the fallback. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. Read-only `/operator` is on the tunnel ([PR #58](https://github.com/artofdream/aea-interactive-design/pull/58) / [#54](https://github.com/artofdream/aea-interactive-design/issues/54)) — **not FR-19**. Owner reboot in progress — this URL is the earlier Knowledge probe, not a post-reboot re-check. A new App tunnel may supersede it before merge. Re-probe before marking ready / pasting as the final live share. Never claim `cafe.artof.link` is Café Fausse.
...
4. **P0 live-proof path**
   ...
   - **Tunnel:** `https://happy-glasses-film.loca.lt/` GET **200** this session (SPA + `/operator` + `/api/health`). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Temporary. **Fallback:** committed clips. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL.
...
5. **Lock video + scenarios.** Use the [script](video-script.md) and scenario menu A–F. Share Knowledge + `https://happy-glasses-film.loca.lt/` (GET **200** this session; **slow (~10–20s)**; **interstitial-possible** in browsers). Clips stay fallback. Old `https://real-goats-shop.loca.lt/` is **stale**. Still a look at the in-repo App, not `cafe.artof.link`.
...
- Tunnel URL written on the [Brief](brief.md): `https://happy-glasses-film.loca.lt/` GET **200** this session (SPA + `/operator` + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on). Temporary. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL.
...
flowchart TB
  ...
  subgraph APP["Café Fausse App — after GET 200 this session, not a public host"]
    ...
    TUN["happy-glasses-film.loca.lt GET 200 · slow ~10-20s · interstitial-possible"] -.-> FLASK
  end
