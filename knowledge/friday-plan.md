...
**App tunnel this session:** `https://happy-glasses-film.loca.lt/` — earlier probe GET **200** (SPA HTML). `https://happy-glasses-film.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. **Not** `cafe.artof.link`. Clips stay the fallback. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. Read-only `/operator` is on the tunnel ([PR #58](https://github.com/artofdream/aea-interactive-design/pull/58) / [#54](https://github.com/artofdream/aea-interactive-design/issues/54)) — **not FR-19**. This-session re-probe: root GET **503** (~15s); `/api/health` GET **503** Tunnel Unavailable (~12s). **Not** the live share. Earlier Knowledge probe was GET **200** (**slow (~10–20s)**; **interstitial-possible**). Wait for App to post a new URL and a GET **200**. Clips stay fallback. Never claim `cafe.artof.link` is Café Fausse.
...
| Journey **J1–J8 PASS** (cts-ai, DB up) + **J9 / NFR-3 / NFR-8 PASS** (Vite-only; Edge `probe-nfr/` 375×812 home, 768×1024 menu, 1280×800 reservations) — [Coverage](coverage.md) / [#40](https://github.com/artofdream/aea-interactive-design/issues/40) / [#44](https://github.com/artofdream/aea-interactive-design/issues/44) | Reservations / `cafe-pg` on the tunnel — **not** claimed (no write probe this session). Old `https://real-goats-shop.loca.lt/` is **stale**. Vite `:5173` from this Knowledge VM **Unknown** |
| App tunnel `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA HTML) + `/operator` + `/api/health` GET **200** `{"ok":true}` | **Slow (~10–20s)**; **interstitial-possible** in browsers. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai — **not** `cafe.artof.link`. Old `https://real-goats-shop.loca.lt/` is **stale** |
...
3. **App tunnel to share.** `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA HTML). Health `https://happy-glasses-film.loca.lt/api/health` GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. Written on the [Brief](brief.md). Clips stay fallback. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. **Do not** open `cafe.artof.link` and call it Café Fausse.
4. **P0 live-proof path**
   ...
   - **Tunnel:** `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA + `/operator` + `/api/health`). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Temporary. **Fallback:** committed clips. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL.
...
5. **Lock video + scenarios.** Use the [script](video-script.md) and scenario menu A–F. Share Knowledge + clips until a new GET **200** (earlier `https://happy-glasses-film.loca.lt/` GET **200**; **slow (~10–20s)**; **interstitial-possible** in browsers). Clips stay fallback. Old `https://real-goats-shop.loca.lt/` is **stale**. Still a look at the in-repo App, not `cafe.artof.link`.
6. **Honesty line.** J1–J8 PASS is local cts-ai UX while the DB was up. J9 / NFR-3 / NFR-8 **PASS** is Vite-only (Edge `probe-nfr/` screenshots + theme.css) — not Flask+Postgres. **NFR-7** is **partial** (Edge + Firefox home; Chrome not installed; Safari Unknown). **NFR-1** / **NFR-2** stay not-claimed-met. Do not say the SRS broadband budgets are met. Earlier tunnel GET **200** is **not** live now (this-session **503**). Wait for a new App URL and GET **200**. Do not claim `cafe-pg` writes from the old health GET.
...
- Tunnel URL written on the [Brief](brief.md): `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA + `/operator` + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on). Temporary. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL.
...
flowchart TB
  ...
  subgraph APP["Café Fausse App — after GET 200 this session, not a public host"]
    ...
    TUN["happy-glasses-film.loca.lt earlier 200 · this-session 503 · clips fallback"] -.-> FLASK
  end
...
- **App** is the in-repo restaurant. Tunnel this session: `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA + `/operator` + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on). Temporary localtunnel to Flask on cts-ai — not a public host, not `cafe.artof.link`. Clips (`clips/01-home-menu.mp4`, `clips/02-happy-book.mp4`) are the fallback look. Old `https://real-goats-shop.loca.lt/` is **stale**.
...
1. Knowledge HTTPS tab open. App tunnel tab: `https://happy-glasses-film.loca.lt/` (this-session GET **503**; earlier GET **200**; **slow (~10–20s)**; **interstitial-possible** in browsers). Clips ready as fallback. Not the stale `real-goats-shop` URL.
2. Script + scenario A–F locked. Slides = 8-slide cut unless the room wants the long outline.
3. Say J1–J8 **PASS** (cts-ai, DB up) and J9 / NFR-3 / NFR-8 **PASS** (Vite-only Edge screenshots + theme.css) from [Coverage](coverage.md) / [#40](https://github.com/artofdream/aea-interactive-design/issues/40) / [#44](https://github.com/artofdream/aea-interactive-design/issues/44). Say **NFR-7** is **partial** (Edge + Firefox home; Chrome not installed; Safari Unknown). Say not-claimed-met for **NFR-1** / **NFR-2**. Share Knowledge + clips until a new GET **200** (earlier `https://happy-glasses-film.loca.lt/` GET **200**; **slow (~10–20s)**; **interstitial-possible** in browsers). Clips stay fallback. Old `https://real-goats-shop.loca.lt/` is **stale**. Do not claim `cafe-pg` writes from the health GET.
