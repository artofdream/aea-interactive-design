...
- **Newsletter outbound via Amazon SES** ([#135](https://github.com/artofdream/aea-interactive-design/issues/135)) — sponsor-approved Future **after** **FR-15** / **FR-16** validate + store. Scope: From domain (`cafe.artof.link` or `@artof.link`), SES sandbox exit / production sending, unsubscribe. Weekend Lightsail staging [#57](https://github.com/artofdream/aea-interactive-design/issues/57) may host IAM/SES credentials. Grade floor stays **store-only** until send is live and probed. The 2026-09-12 App public-host probe **confirmed** store-only: `POST /api/newsletter` → **201**; `email_delivery` skipped / `"email not configured"`; no `SES_FROM_EMAIL` / `SES_REGION` / `PUBLIC_BASE` on the container; inbox receive **N/A**. Send is still **not** live. Not a new FR; not **FR-19**. Related parked email-identity / verbatim-case Future is [#37](https://github.com/artofdream/aea-interactive-design/issues/37) (identity rule, not outbound mail). App tracker #135 stays open; this page is the Knowledge list note only. [#198](https://github.com/artofdream/aea-interactive-design/issues/198)
...
- [Glossary](glossary.md) — first-class page (also in the top nav). Older stub: [future/glossary.md](future/glossary.md).
