...
| 1:00–4:00 | **HLD dual-env** | Prefer [local HLD](assets/hld-local.svg) + [MSAIE staging HLD](assets/hld-aws-msaie.svg). Knowledge is GitHub Pages. Prefer `https://cafe.artof.link/` (MSAIE staging — on-box Postgres, not a shared RDS). Off-camera: Route53 A → Lightsail, Caddy + LE. The `*.artof.link` ELB wildcard is **not** Café Fausse. Temporary staging — not production forever. Longer-term hosting stays #22. History still: [AWS staging SVG](assets/hld-aws-staging.svg). |
