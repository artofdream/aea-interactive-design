...
| App tunnel tonight | `https://tender-rings-kick.loca.lt/` GET **200** this session (SPA HTML, title Café Fausse). `/api/health` GET **200** `{"ok":true}`. Temporary localtunnel to Flask on cts-ai. **Not** `cafe.artof.link`. Clips stay fallback. Old `https://nine-teams-try.loca.lt/` GET **503**. |
...
| **Local / tunnel** | Café Fausse App on `main`. Tunnel this session: `https://tender-rings-kick.loca.lt/` GET **200** (SPA + `/api/health`). Temporary localtunnel to Flask on cts-ai. Silent clips are the **fallback** look. | Not proof that reservations work on a public host. Not `cafe.artof.link`. |
