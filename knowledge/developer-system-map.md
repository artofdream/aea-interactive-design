...
**Honesty locks:** **Local (dev)** + **Docker** `cafe-pg` (`postgres:16`) · **on-box Postgres** on MSAIE staging (not a shared RDS; no florist Path B) · routes are app-factory `@app.get` / `@app.post` — **not Flask Blueprints** · no `/api/gallery` · newsletter **store-only** unless SES env is set · **fail-closed** if DB is down · on camera say **MSAIE staging** · **HTTP only** (no event bus).
...
Browser (React SPA)
  ...
  └── /operator → read-only demo snapshot
         │
         ▼
Flask app factory (`create_app` — @app.get / @app.post, not Blueprints)
  ├── /api/* JSON
...
| **Backend** | Slots, reservations, newsletter store, fail-closed, freeze JSON APIs | Inventing menu/gallery copy outside freeze; Flask Blueprints |
...
Registered on the Flask **app factory** in `backend/cafe_fausse/__init__.py` (`@app.get` / `@app.post`). **Not Flask Blueprints** (Hiren’s cut uses Blueprints; this repo does not).
...
| Flask app / routes | `backend/cafe_fausse/__init__.py` app factory (`@app.get`/`@app.post`) | Wire HTTP → services (**not Flask Blueprints**) |
...
6. Routes are **`@app.get` / `@app.post` on the app factory** — **not Flask Blueprints**.
7. `/api/operator` is a demo/recording helper, **not FR-19**.
8. On camera: **MSAIE staging**, not Lightsail jargon.
9. **Local (dev) + Docker** — Docker here is `postgres:16` as `cafe-pg` (or a local install). Not the product name.

---

*Sources: New Bot App inventory tip `73d202d`; `backend/schema.sql`; `frontend/src/pages/`; Flask app factory in `backend/cafe_fausse/__init__.py` (`@app.get`/`@app.post`, not Blueprints); README Docker `cafe-pg`. Unknown until probed stays labeled if anything drifts after tip. [Parts 3–5 materials](parts-345-materials.md) · [Handoff mapping](parts-345-handoff-mapping.md) (not spoken on camera).*
