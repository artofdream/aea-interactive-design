...
**App tunnel this session:** `https://real-goats-shop.loca.lt/` — GET **200** (SPA HTML). `https://real-goats-shop.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** ~15s; `/api/health` **200** ~15s. **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. **Not** `cafe.artof.link`. Clips stay the fallback. Old `https://tender-rings-kick.loca.lt/` is stale — do not keep as the live share URL. Never claim `cafe.artof.link` is Café Fausse. FS v0.1 extras = Future; presentation = [Coverage](coverage.md). `quantic-grader` remains an owner step.

## Friday 2026-09-04 19:00 Europe/Berlin (score 5)

**App tunnel this session:** `https://real-goats-shop.loca.lt/` GET **200** (SPA HTML). `/api/health` GET **200** `{"ok":true}`. Knowledge GET: root **200** ~15s; `/api/health` **200** ~15s. **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. Clips stay fallback. **Not** `cafe.artof.link`.

- **Primary live surface:** `https://knowledge.cafe.artof.link/` (HTTPS GET **200** this session).
- **App tunnel (share this tab):** `https://real-goats-shop.loca.lt/` — GET **200** this session (SPA HTML, title Café Fausse). Health `https://real-goats-shop.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** ~15s; `/api/health` **200** ~15s. **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Temporary; not snappy, not always-on. Old `https://tender-rings-kick.loca.lt/` is stale — do not keep as the live share URL.
...
Silent ~30s demos of the restaurant MVP (Café Fausse App). **Fallback** if `https://real-goats-shop.loca.lt/` is **slow (~10–20s)**, shows a localtunnel interstitial in browsers, or drops. They are not a probe that reservations work on a public host, and they are not Café Fausse at `cafe.artof.link`. The live share tunnel this session is `https://real-goats-shop.loca.lt/` (GET **200** SPA + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on) — temporary localtunnel to Flask on cts-ai. Do not keep `https://tender-rings-kick.loca.lt/` as the live share URL.
...
**App tunnel this session:** `https://real-goats-shop.loca.lt/` GET **200** (SPA + `/api/health`). Knowledge GET: root **200** ~15s; `/api/health` **200** ~15s. **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. Clips stay fallback. **Not** `cafe.artof.link`. Do not keep old `https://tender-rings-kick.loca.lt/` as the live share URL.

Wed 2026-09-02 locked **1, 3, 4** (not 2). Those decisions still hold:

1. **Demo:** Knowledge brief + clips; App tunnel `https://real-goats-shop.loca.lt/` GET **200** this session (SPA + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on). **Not** `cafe.artof.link` (AWS ELB, not our restaurant; hosting is [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22)).
