...
**Live share this session (prefer):** `https://cafe.artof.link/` — GET **200** (SPA HTML). `https://cafe.artof.link/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~0.5s); `/operator` **200** (~0.6s); `/api/operator` **200** (~0.5s); `/api/health` **200** `{"ok":true}` (~0.6s). This agent GET: root **200** (~0.04s); `/operator` **200** (~0.03s); `/api/operator` **200** (~0.04s); `/api/health` **200** `{"ok":true}` (~0.04s). TLS CN/SAN `cafe.artof.link`, Let’s Encrypt, `notAfter=2026-12-04`. HTTP **308** to HTTPS. DNS A `54.165.102.60`. Lightsail staging (us-east-1, [#57](https://github.com/artofdream/aea-interactive-design/issues/57)) — **not** a permanent hosting claim. Staging stays **up** until the owner explicitly requests tear-down. Monday **16:00 CET** is evaluate-only (not automatic tear-down). Quantic grader host need remains **Unknown** until their email. Longer-term hosting stays [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22). Do not over-claim production forever. Fast this session; still staging. Clips stay the fallback. **Interim backup** (same host, not the primary paste): `https://54-165-102-60.sslip.io/`. Old `https://shaky-deer-drive.loca.lt/` is **stale** (Knowledge GET **503**; this agent timed out, curl 28). Old `https://happy-glasses-film.loca.lt/` and `https://real-goats-shop.loca.lt/` stay **stale**. Read-only `/operator` is on the share ([PR #58](https://github.com/artofdream/aea-interactive-design/pull/58) / [#54](https://github.com/artofdream/aea-interactive-design/issues/54)) — **not FR-19**. FS v0.1 extras = Future; presentation = [Coverage](coverage.md). `quantic-grader` remains an owner step.

## Friday 2026-09-04 19:00 Europe/Berlin (score 5)

**Live share this session (prefer):** `https://cafe.artof.link/` GET **200** (SPA HTML). `/api/health` GET **200** `{"ok":true}`. Knowledge GET: root **200** (~0.5s); `/operator` **200** (~0.6s); `/api/operator` **200** (~0.5s); `/api/health` **200** `{"ok":true}` (~0.6s). Lightsail staging (#57) — weekend recording window, not production forever. Fast this session; still staging. Clips stay fallback. **Interim backup:** `https://54-165-102-60.sslip.io/`. Old `https://shaky-deer-drive.loca.lt/`, `https://happy-glasses-film.loca.lt/`, and `https://real-goats-shop.loca.lt/` are **stale**.

- **Primary live surface:** `https://knowledge.cafe.artof.link/` (HTTPS GET **200** this session).
- **App live share (prefer this tab):** `https://cafe.artof.link/` — GET **200** this session (SPA HTML, title Café Fausse). Health `https://cafe.artof.link/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~0.5s); `/operator` **200** (~0.6s); `/api/operator` **200** (~0.5s); `/api/health` **200** `{"ok":true}` (~0.6s). Lightsail staging (#57) — not permanent. Interim backup: `https://54-165-102-60.sslip.io/`. Old `https://shaky-deer-drive.loca.lt/`, `https://happy-glasses-film.loca.lt/`, and `https://real-goats-shop.loca.lt/` are **stale**.
- **Fallback:** committed clips below (if staging drops).
- **Operator view (recording helper):** `https://cafe.artof.link/operator` — Knowledge GET **200** (~0.6s). Read-only customers + reservations so you can see the DB after a booking. **Not** an admin console (no CRUD / cancel). **Not FR-19.** Landed on `main` via [PR #58](https://github.com/artofdream/aea-interactive-design/pull/58) / [issue 54](https://github.com/artofdream/aea-interactive-design/issues/54). Interim backup: `https://54-165-102-60.sslip.io/operator`.
- **Do not** treat this hostname as production forever. Longer-term hosting stays [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22).
...
**Where (live share this session, prefer):** `https://cafe.artof.link/operator` — Knowledge GET **200** this session (~0.6s). Lightsail staging (#57) — weekend recording window, not permanent. Interim backup: `https://54-165-102-60.sslip.io/operator`.
...
**Ready (do not over-claim):** restaurant MVP on `main`; Coverage maps every freeze ID; Knowledge HTTPS GET **200** this session; prefer live share `https://cafe.artof.link/` GET **200** (SPA + `/operator` + `/api/health`; Lightsail staging #57 — not production forever); two silent clips as fallback. Interim backup `https://54-165-102-60.sslip.io/`. Longer-term hosting stays [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22).
...
Silent ~30s demos of the restaurant MVP (Café Fausse App). **Fallback** if `https://cafe.artof.link/` drops. They are not a probe that every reservation write works without a this-session write probe, and they are not a permanent hosting claim. Prefer live share `https://cafe.artof.link/` (GET **200** SPA + `/operator` + `/api/health`; Lightsail staging #57 — weekend recording window, not production forever). Interim backup: `https://54-165-102-60.sslip.io/`. Old `https://shaky-deer-drive.loca.lt/`, `https://happy-glasses-film.loca.lt/`, and `https://real-goats-shop.loca.lt/` are **stale**.
...
|---|---|---|
| Café Fausse Knowledge | `knowledge.cafe.artof.link` | HTTPS GET **200** this session (2026-09-05); HTTP **301** to HTTPS. |
| Café Fausse App | `cafe.artof.link` | Prefer live share: HTTPS GET **200** this session (SPA + `/operator` + `/api/health`). TLS CN/SAN `cafe.artof.link`, Let’s Encrypt `notAfter=2026-12-04`. DNS A `54.165.102.60`. Lightsail staging (#57) — weekend recording window, not production forever. Longer-term hosting stays [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22). |

App on cts-ai (not this agent VM): Vite `http://127.0.0.1:5173`, Flask `:5000`, `cafe-pg` Postgres. **Live share this session (prefer):** `https://cafe.artof.link/` GET **200** (SPA HTML); `/api/health` GET **200** `{"ok":true}`. Knowledge GET: root **200** (~0.5s); `/operator` **200** (~0.6s); `/api/operator` **200** (~0.5s); `/api/health` **200** `{"ok":true}` (~0.6s). Lightsail staging (#57) — weekend recording window, not production forever. Fast this session; still staging. Interim backup: `https://54-165-102-60.sslip.io/`. This Knowledge VM did not reach Vite on `:5173`. Journey **J1–J8 PASS** recorded 2026-09-02 while the DB was up; **J9 / NFR-3 / NFR-8 PASS** later on Vite-only `:5173` (Edge `probe-nfr/` screenshots + theme.css; not Flask+Postgres). **NFR-7** **partial** (Edge all routes with screenshots; Firefox home; Chrome not installed; Safari Unknown). After the J1–J8 handoff, App reported `cafe-pg` unreachable — do not claim reservations on the share without a write probe. Old `https://shaky-deer-drive.loca.lt/`, `https://happy-glasses-film.loca.lt/`, and `https://real-goats-shop.loca.lt/` are **stale**.
...
## Friday room — remote default, then live share

Not **2:** `quantic-grader` stays an owner step.

**Live share this session (prefer):** `https://cafe.artof.link/` GET **200** (SPA + `/operator` + `/api/health`). Knowledge GET: root **200** (~0.5s); `/operator` **200** (~0.6s); `/api/operator` **200** (~0.5s); `/api/health` **200** `{"ok":true}` (~0.6s). Lightsail staging (#57) — weekend recording window, not production forever. Fast this session; still staging. Clips stay fallback. **Interim backup:** `https://54-165-102-60.sslip.io/`. Old `https://shaky-deer-drive.loca.lt/`, `https://happy-glasses-film.loca.lt/`, and `https://real-goats-shop.loca.lt/` are **stale**.

Wed 2026-09-02 locked **1, 3, 4** (not 2). Those decisions still hold:

1. **Demo:** Knowledge brief + clips; prefer `https://cafe.artof.link/` GET **200** (SPA + `/operator` + `/api/health`; Lightsail staging #57 — not production forever). Interim backup `https://54-165-102-60.sslip.io/`. Longer-term hosting stays [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22).
...
4. **Presentation:** [Coverage](coverage.md) freeze map + Journey table. J1–J8 PASS (cts-ai, DB up). J9 / NFR-3 / NFR-8 **PASS** (Vite-only Edge screenshots + theme.css). **NFR-7** **partial** (Edge + Firefox home; Chrome not installed; Safari Unknown). **NFR-1** / **NFR-2** stay not-claimed-met. Live share URL written after GET **200** this session.
