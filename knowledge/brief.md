...
**App tunnel this session:** `https://happy-glasses-film.loca.lt/` — earlier probe GET **200** (SPA HTML). `https://happy-glasses-film.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. **Not** `cafe.artof.link`. Clips stay the fallback. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. Read-only `/operator` is on the tunnel ([PR #58](https://github.com/artofdream/aea-interactive-design/pull/58) / [#54](https://github.com/artofdream/aea-interactive-design/issues/54)) — **not FR-19**. This-session re-probe: root GET **503** (~15s); `/api/health` GET **503** Tunnel Unavailable (~12s). **Not** the live share. Earlier Knowledge probe was GET **200** (**slow (~10–20s)**; **interstitial-possible**). Wait for App to post a new URL and a GET **200**. Clips stay fallback. Never claim `cafe.artof.link` is Café Fausse. FS v0.1 extras = Future; presentation = [Coverage](coverage.md). `quantic-grader` remains an owner step.

## Friday 2026-09-04 19:00 Europe/Berlin (score 5)

**App tunnel this session:** `https://happy-glasses-film.loca.lt/` this-session re-probe GET **503** Tunnel Unavailable (root ~15s; `/api/health` ~12s). **Not** the live share. Earlier Knowledge probe GET **200** (SPA HTML). `/api/health` earlier GET **200** `{"ok":true}`. Knowledge GET (earlier): root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** when it was up. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. Clips stay fallback. **Not** `cafe.artof.link`. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. Wait for App to post a new URL and a GET **200**.

- **Primary live surface:** `https://knowledge.cafe.artof.link/` (HTTPS GET **200** this session).
- **App tunnel (share this tab):** `https://happy-glasses-film.loca.lt/` — earlier probe GET **200** this session (SPA HTML, title Café Fausse). Health `https://happy-glasses-film.loca.lt/api/health` — GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Temporary; not snappy, not always-on. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL.
...
Silent ~30s demos of the restaurant MVP (Café Fausse App). **Fallback** if `https://happy-glasses-film.loca.lt/` is **slow (~10–20s)**, shows a localtunnel interstitial in browsers, or drops. They are not a probe that reservations work on a public host, and they are not Café Fausse at `cafe.artof.link`. The earlier-probed tunnel `https://happy-glasses-film.loca.lt/` (earlier GET **200** SPA + `/operator` + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on) — temporary localtunnel to Flask on cts-ai. Do not keep `https://real-goats-shop.loca.lt/` as the live share URL.
...
App on cts-ai (not this agent VM): Vite `http://127.0.0.1:5173`, Flask `:5000`, `cafe-pg` Postgres. **Tunnel this session:** `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA HTML); `/api/health` GET **200** `{"ok":true}`. Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Temporary localtunnel to Flask on cts-ai. Not snappy. Not always-on. **Not** `cafe.artof.link`. This Knowledge VM did not reach Vite on `:5173`. Journey **J1–J8 PASS** recorded 2026-09-02 while the DB was up; **J9 / NFR-3 / NFR-8 PASS** later on Vite-only `:5173` (Edge `probe-nfr/` screenshots + theme.css; not Flask+Postgres). **NFR-7** **partial** (Edge all routes with screenshots; Firefox home; Chrome not installed; Safari Unknown). After the J1–J8 handoff, App reported `cafe-pg` unreachable — do not claim reservations on the tunnel without a write probe. Old `https://real-goats-shop.loca.lt/` is **stale** — do not keep as the live share URL. Do **not** claim `cafe.artof.link` is the restaurant.
...
**App tunnel this session:** `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA + `/operator` + `/api/health`). Knowledge GET: root **200** (~13.5s); `/operator` **200** (~16s); `/api/operator` **200** (~11s); `/api/health` **200** `{"ok":true}` (~12s). **Slow (~10–20s)**; **interstitial-possible** in browsers. Short timeouts can look like failure. Not snappy. Not always-on. Temporary localtunnel to Flask on cts-ai. Clips stay fallback. **Not** `cafe.artof.link`. Do not keep old `https://real-goats-shop.loca.lt/` as the live share URL.

Wed 2026-09-02 locked **1, 3, 4** (not 2). Those decisions still hold:

1. **Demo:** Knowledge brief + clips; App tunnel `https://happy-glasses-film.loca.lt/` earlier probe GET **200** (SPA + `/operator` + `/api/health`; **slow (~10–20s)**; **interstitial-possible** in browsers — not snappy, not always-on). **Not** `cafe.artof.link` (AWS ELB, not our restaurant; hosting is [Future #22](https://github.com/artofdream/aea-interactive-design/issues/22)).
