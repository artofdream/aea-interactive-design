...
Use with [Coverage](coverage.md) (every freeze ID) and the [Friday plan](friday-plan.md) diagram. Spoken timing lives in the [video script](video-script.md). Saturday **locked** VIDEO (five parts, #97): [Saturday](meeting-saturday.md) / [Presentation](presentation.md). This file stays the 8/12-slide outline — not a second talk track.
...
| 9 | **Live or fallback** | HTTPS Knowledge. Prefer `https://cafe.artof.link/`. Clips if staging drops. | Staging #57, not production forever. |
...
- “Production-forever restaurant at `cafe.artof.link`.” Prefer it as the weekend staging share (#57).
