...
Skills / process (Grok + Café): Knowledge Honesty lists shared Grok skills this lane adopts (`pr-train-rebase`, `honesty-ledger-gate`, `companion-plain-docs`, `persona-journey-validation`), the Café Pages ratchet (`knowledge-pages-ratchet`), and AEA **Keep learning and apply** ([AEA #434](https://gitlab.com/artof-group/adaptive-experience-architecture/-/work_items/434); Café [#213](https://github.com/artofdream/aea-interactive-design/issues/213)): learn from Café/AEA history → encode into skills/matrix → next PR **applies** it. Why/What: when the build teaches something, write it into the harness before the next loop. `knowledge-pages-ratchet` is the apply/publish loop (probe closes the claim). A skill is process memory — it does not prove Live or a freeze ID without a this-session probe. The #434 row is **Documented** until a Pages probe shows the matrix live. See [Honesty](knowledge/honesty.md) (section **Skills / process (Grok + Café)**) and issue [#203](https://github.com/artofdream/aea-interactive-design/issues/203). This log does not close [#204](https://github.com/artofdream/aea-interactive-design/issues/204) (Pages ratchet skill body).
