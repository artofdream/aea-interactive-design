...
**Probe 2026-09-05 Europe/Berlin (this session):** `GET https://cafe.artof.link/` **200** (title Café Fausse; remote IP `54.165.102.60`); `/api/health` **200** `{"ok":true}`; `/operator` **200**; `/api/operator` **200**. TLS VERIFY_OK, CN `cafe.artof.link`, Let’s Encrypt `notAfter=2026-12-04`. DNS **A** `54.165.102.60` (Route53 TTL 60; Lightsail `cafe-fausse-staging` us-east-1; Caddy LE). HTTP → **308** HTTPS. Interim backup: `GET https://54-165-102-60.sslip.io/` **200**; `/api/health` **200** `{"ok":true}`. Knowledge: `GET https://knowledge.cafe.artof.link/` **200** (GitHub Pages; CNAME `artofdream.github.io`); HTTP → **301** HTTPS; TLS CN/SAN match, Let’s Encrypt `notAfter=2026-11-30`. Tunnel `https://shaky-deer-drive.loca.lt/` **timeout** (curl 28, 15s) — demoted; not live share. AEA RDS untouched. Newsletter = store only (**FR-15** / **FR-16**); no outbound mailer. Staging stays **up** until the owner explicitly requests tear-down. Monday **16:00 CET** is an evaluate-only checkpoint (not automatic tear-down). Quantic grader host need remains **Unknown** until their email.
